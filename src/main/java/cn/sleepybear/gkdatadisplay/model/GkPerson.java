 ...
 *
 * @author sleepybear
 * @date 2024/08/19 19:21
 */
@EqualsAndHashCode(callSuper = true)
@Data
@TableName(value = "GK_LUQU_DATA", autoResultMap = true)
public class GkPerson extends BaseModel implements Serializable {
    @Serial
    private static final long serialVersionUID = -7789340782198702825L;
    ...
    /**
     * 将这个 class 的所有字段属性变成小写并 print，使用反射。最终格式是 class 的完整格式
     */
    public void printClassToLowerCase1() {
        StringBuilder totalClassStr = new StringBuilder();
        // 构建 class 头和字段
        totalClassStr.append("public class ").append(this.getClass().getSimpleName()).append(" {\n");
        Field[] fields = this.getClass().getDeclaredFields();
        for (Field field : fields) {
            totalClassStr.append("    private String ").append(field.getName().toLowerCase()).append(";\n");
        }
        totalClassStr.append("}\n");
        System.out.println(totalClassStr);
    }

    public static void main(String[] args) {
        GkPerson gkPerson = new GkPerson();
        gkPerson.printClassToLowerCase1();
    }
}
